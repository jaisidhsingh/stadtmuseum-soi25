"""
SOI API - MediaPipe pose + api3-compatible frontend contract.

This module keeps the endpoint contract expected by the current frontend:
- /characters
- /segment (returns context_id and original silhouette)
- /stylize
- /backgrounds
- /composite
- /share/*
...
Unlike api3, this module does not use OpenPose. Pose estimation is done via
the local MediaPipe PoseEstimator stack.
"""
...
# Add parent directory so we can import generate_silhouettes helpers.
...
def load_links(links_dir: Path) -> dict:
    ...
    a character's links/ directory. Missing bilateral parts are auto-generated
    by mirroring their counterpart.
    ...
    for missing_name, source_name in MIRROR_PAIRS.items():
        ...
        flipped_image = cv2.flip(source["image"], 1)
...
def generate_silhouette_rgba(
    ...
    Reuses parse_keypoints / adjust_link_direction / flip_link /
    get_link_transformation_matrix from generate_silhouettes.py.
    """
    w, h = canvas_size

    # Work on a copy so flips do not persist across calls.
    ...
    for link_name in ordered:
        ...
        if (
            "relative_length" in link_data
            and "relative_length" in kp_data
        ...
        if warped.shape[2] == 4:
            ...
            out_bgr = (
                link_bgr * link_alpha
                + canvas_bgr * canvas_alpha * (1 - link_alpha)
            ) / out_alpha_safe
...
app = FastAPI(title="SOI API", version="3.0.0")


def _list_characters() -> List[str]:
    """List character folders with links/links.csv under backend/cutouts."""
    chars = []
...
for char_name in _list_characters():
    ...
    if char_links_dir.is_dir():
        app.mount(
            f"/assets/{char_name}",
            StaticFiles(directory=str(char_links_dir)),
            name=f"assets_{char_name}",
        )


# ---------------------------------------------------------------------------
# Lazy singletons and runtime caches
# ---------------------------------------------------------------------------

_pose_engine: Optional[PoseEstimator] = None
_seg_engine: Optional[SegmentationEngine] = None
_engine_cache: Dict[str, dict] = {}
_stylize_runtime_cache: Dict[str, Dict[str, Any]] = {}
...
# Privacy-first startup cleanup: remove stale exported share files.
...
def get_links(base_name: str) -> dict:
    ...
    if base_name not in _engine_cache:
        ...
        if preferred_dir.is_dir():
            ...
            raise FileNotFoundError(f"Character directory not found: {preferred_dir}")
...
# ---------------------------------------------------------------------------
# Pose helper (MediaPipe path)
# ---------------------------------------------------------------------------

def run_pose_on_image(image_path: Path) -> dict:
    """Run MediaPipe pose estimation on a single image and return result dict."""
    pe = get_pose_engine()
    img = Image.open(str(image_path)).convert("RGB")

    logger.info(f"Running MediaPipe pose on {image_path.name}...")
    pose_result = pe.detect_poses(img, include_hands=True)

    if not pose_result:
        return {"people": [], "canvas_width": img.width, "canvas_height": img.height}

    pose_result.setdefault("people", [])
    pose_result.setdefault("canvas_width", img.width)
    pose_result.setdefault("canvas_height", img.height)
    return pose_result
...
def _stylize_from_context(
    ...
    if "pose_result" not in runtime:
        logger.info(f"[stylize:{context_id}] Running MediaPipe pose (first stylize call)...")
        runtime["pose_result"] = run_pose_on_image(input_path)
    ...
    people = pose_result.get("people", [])
    canvas_w = pose_result.get("canvas_width", original_photo.width)
    canvas_h = pose_result.get("canvas_height", original_photo.height)

    if not people:
        logger.warning(f"[stylize:{context_id}] Pose engine detected no people.")
    ...
    base_sil: Image.Image = runtime["base_sil"]

    # Mirror current api3 behavior: preserve the full silhouette base and
    # alpha-composite warped parts on top.
    ...
    warp_tmp_path.unlink(missing_ok=True)
...
async def get_backgrounds():
    ...
    for dict_key, bg_info in BACKGROUNDS.items():
        ...
        bg_list.append(
            {
                "id": dict_key,
                "title": bg_info.get("title", dict_key),
...
async def segment_image(
    ...
    """
    _ = (base_name, parts_to_warp, character_mapping)
    ...
    try:
        ...
        sil_tmp_path.unlink(missing_ok=True)
        ...
        # Prime stylize runtime with already-computed data.
...
async def stylize_image(request: StylizeRequest):
    ...
    Pose/U2Net data is computed once per context and reused for subsequent
    stylization updates in the same session.
    """
    try:
